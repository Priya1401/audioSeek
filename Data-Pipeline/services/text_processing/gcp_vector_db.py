...
class GCPVectorDB(VectorDBInterface):
    ...
    def __init__(
        ...
        self.index_id = index_id or os.getenv('GCP_INDEX_ID',
                                              'audioseek-embeddings')
        ...
        logger.info(
            f"Initialized GCP Vector DB: project={project_id}, index={self.index_id}")
        ...
        try:
            ...
            if self.index_id:
                try:
                    ...
                    logger.warning(
                        f"Could not connect to index {self.index_id}: {e}")
                    logger.info(
                        "Index will need to be created via GCP Console or Terraform")
            ...
            if self.index_endpoint_id:
                try:
                    ...
                    logger.info(
                        f"Connected to existing endpoint: {self.index_endpoint_id}")
                except Exception as e:
                    logger.warning(
                        f"Could not connect to endpoint {self.index_endpoint_id}: {e}")
                    self.index_endpoint = None
    ...
    def add_documents(self, embeddings: List[List[float]],
        ...
        if not self.index:
            raise RuntimeError(
                "Index not initialized. Please create the index in GCP Console first.")

        logger.info(
            f"Adding {len(embeddings)} documents to Vertex AI Vector Search")
        ...
        try:
            ...
            if hasattr(self.index, 'upsert_datapoints'):
                ...
                logger.warning(
                    "Direct upsert not available, using batch upload")
            ...
            logger.info(
                "Note: You may need to create the index first via GCP Console")
            raise
    ...
    def search(self, query_embedding: List[float], top_k: int = 5) -> List[
        ...
        logger.info(
            f"Searching for top {top_k} results in Vertex AI Vector Search")
        ...
        try:
            ...
            if self.index_endpoint and hasattr(self.index_endpoint,
                                               'find_neighbors'):
                # Use endpoint for search
                deployed_index_id = None
                if self.index_endpoint.deployed_indexes:
                    deployed_index_id = self.index_endpoint.deployed_indexes[
                        0].id
                ...
                logger.warning(
                    "Direct search not available - using placeholder")
                results = []

            # Format results
            formatted_results = []
            if results and len(results) > 0:
                for result in results[0] if isinstance(results,
                                                       list) else results:
            ...
            logger.info(
                "Note: Make sure the index is deployed and endpoint is configured")
            raise
    ...
    def verify_connection(self) -> bool:
        ...
        try:
            if not self.index:
                logger.warning(
                    "Index not initialized - connection verification failed")
                return False
